...
/**
 * If some frame manipulating machinery is "SemiDetCompiled" it means that it produces at most one frame of output for each frame of input. Thus it has a specific run method that
 * takes just one frame and returns one frame or null.
 */
public abstract class SemiDetCompiled extends Compiled {
    ...
    /**
     * Note: this method is provided so that contexts which expect a nonDet or multi mode can use this. However, it would be more efficient for compiled contexts to adapt when they
     * are passing frames to a semiDetCompiled and use the runSemiDet instead.
     */
    @Override
