...
/**
 * A ModedRuleBaseCollection is a collection of ModedRuleBases that have the same predicate identifier. Each ModedRuleBase is distinguished from the others by its predicate mode
 */
...
public class ModedRuleBaseCollection {
    ...
    /**
     * Keeps the unconverted rules so that we can create more ModedRuleBases on the fly (and insert all rules into them). This ArrayList contains instances of InsertionInfo.
     */
    ...
    /**
     * Helper method for 'on the fly' creation of an additional ModedRuleBase for a mode which is not declared, but can be supported as a special case of a declared mode.
     *
     * @vsf+ RuleBaseCollectionBug
     */
    ...
    /**
     * return the ModedRuleBase in this collection that has the "best" mode of execution that allow for bindings
     */
