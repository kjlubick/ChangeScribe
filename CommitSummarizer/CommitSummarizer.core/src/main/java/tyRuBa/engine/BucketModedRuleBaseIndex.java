...
/**
 * A BucketModedRuleBaseIndex has a local and a global BasicModedRuleBaseIndex. All facts and rules that are inserted into this rulebase index will first be inserted into the local
 * rulebase index. If that fails (the predicate identifier is not declared locally index), then they are inserted into the global rulebase index.
 */
