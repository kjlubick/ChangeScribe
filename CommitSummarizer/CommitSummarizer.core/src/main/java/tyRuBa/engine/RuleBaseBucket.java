...
/**
 * A RuleBaseBucekt is conceptual set of fact/rules that are added to a FrontEnd.
 * 
 * A RuleBaseBucket should know how to update its facts when asked to do so by calling update(). Therefore, when using RuleBaseBuckets you are expected to make a subclass and
 * implement the "update" method accordingly.
 * 
 * A rulebasebucket has all capabilities of a query engine. It behaves largely as an alias of its associated FrontEnd.
 * 
 * Note: the RuleBaseBucket outdated flag is already set when it is created. Thus, the first time the contents of the bucket is needed it will be requested to update itself to
 * generate the facts for the first time.
 */
...
public abstract class RuleBaseBucket extends QueryEngine {
    ...
    /**
     * Gets the storage path for this query engine (will be where the factbase is stored
     */
    @Override
    ...
    /**
     * This method must be overriden. It will be called by the query engine when it determines that the contents of the bucket is needed after it has been "setOutdated". This
     * method has to regenerate all the facts in this bucket and reinsert them into the bucket. This method should never be called directly, only by doUpdate, which is triggered by
     * the QueryEngine.
     */
