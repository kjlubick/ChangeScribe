...
/**
 * A MetaBase is a wrapper around a QueryEngine providing facilities for storing metaData into that queryEngine.
 * 
 * @author kdvolder
 * @codegroup metadata
 */
public class MetaBase {
     ...
     * @rule
     * 
     *       1) This method must be at the beginning of any method that dereferences one of the XXXFact variables.
     * 
     *       field(?thisClass,?f),re_name(?f,/Fact$),reads(?M,?f,?accessF) => calls(?M,?thisMethod,?callToMe), isBefore(?callToMe, ?accessF).
     * 
     *       msg: The method ?M must call ?thisMethod before accessing ?f
     * 
    ...
    /**
     * Add the type mappings that allos smooth conversion from java representations for types into TyRuBa terms of type meta.Type.
     * 
