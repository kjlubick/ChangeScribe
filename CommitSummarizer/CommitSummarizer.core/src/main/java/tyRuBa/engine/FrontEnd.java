...
public class FrontEnd extends QueryEngine
    ...
    /**
     * If loadInitFile is true then the FrontEnd will automatically load tyruba's standard init files included in its jar file.
     **/
     ...
     * @param initfile
     * @param clean
     *            Force old files to be deleted, start with a "clean" database.
     */
    ...
    /**
     * This method is like backupFactBase, except that it will only perform the backup in case it is not going to take a long time. Use this method for periodic saves in
     * interactive applications.
     * 
     * As the query engine is less active it will gradually save data to disk in background, eventually saving all data to disk. When all page data has been saved, then fastBackup
     * can run. As long as dirty pages remain in memory fastBackupWill not perform a backup.
     * 
     * Returns a boolean value to indicate whether backup was performed.
     * 
     * @throws BackupFailedException
     *             if backup was attempted but failed for some reason.
     */
    ...
    /**
     * You should not call this, only supposed to be called by the buckets constructor
     */
    ...
    /**
     * You should not call this, only supposed to be called by the buckets "destroy" method
     */
    ...
    /**
     * Perform a partial bucket update. The buckets to be checked and updated are given by bucketColl.
     */
    ...
    /**
     * Simulate a crash of the query engine. This terminates all threads associated with the query engine and makes the query engine unusable.
     * 
    ...
    /**
     * This enables metaData facts for the FrontEnd. You must enableMetaData separately for buckets to get metaData for the declarations entered via buckets.
     * 
     * @codegroup metadata
     */
    @Override
