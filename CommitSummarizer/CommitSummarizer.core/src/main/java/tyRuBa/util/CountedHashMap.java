 ...
 * @author cburns
 *
 *         Hashtable that maintains a 16 bit count of insertions and removals. Once a key has been removed as many times as it has been inserted, the mapping will be destroyed.
 * 
 *         Also has a 16 bit flag field for other information.
 * 
 *         This implementation uses distributed lock synchronization.
 * 
 *         HashMap was reimplemented instead of extended because of memory constraints and lock contention, and because only ints are used as keys.
 * 
 *         In general, threads will only contend on put() and remove(), and if entries are actually created or removed.
 */
