 ...
 * @author cburns
 *
 *         Thread pool class. Allows convenient asynchronous execution of Runnable objects without lots of relatively heavyweight calls to new Thread(runnable).start().
 * 
 *         If the run method of the Runnable throws an unhandled exception or error, it will be caught and ignored.
 */
public class ThreadPool {
    ...
    /**
     * @return the number of jobs that are currently waiting. Actual value may be less if the calling thread has exclusive access, may be more or less if multiple threads can call
     *         delegate().
     */
    public int waiting() {
        return jobs.size();
