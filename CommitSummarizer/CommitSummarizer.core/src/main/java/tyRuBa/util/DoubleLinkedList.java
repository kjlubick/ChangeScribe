 ...
 * @author cburns
 *
 *         DoubleLinkedList class that operates by entry rather than by Object, so that O(1) remove operations are possible.
 * 
 *         There are few error checks. If you do something which is incorrect, such as adding an element which is already on the list, removing an element which is not on the list,
 *         adding the list to itself, etc etc etc, it would most likely result in a NullPointerException, and if not it would break later.
 * 
 *         The DoubleLinkedList is not synchronized.
 * 
 *         The DoubleLinkedList cannot store anything that does not extend DoubleLinkedList.Entry. In order to store things in the list, you should extend DoubleLinkedList.Entry
 *         and insert those objects into the list. This reduces memory overhead and simplifies code. It also enforces the operation by entry paradigm.
 */
