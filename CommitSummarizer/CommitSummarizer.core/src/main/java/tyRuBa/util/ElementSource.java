...
/**
 * An element source is similar to a java.util.Enumeration. Except that it can sometimes be temporarily out of elements. A java.util.Enumeration on the other hand either has more
 * elements or it has not.
 * 
 * Elements source have three possible conditions: ELEMENT_READY : I can produce an element now. NO_ELEMENTS_READY : I cannot produce an element now. NO_MORE_ELEMENTS : I cannot
 * produce any more elements, not now AND not in the future.
 * 
 * It is required that an element source discriminates at least between NO_ELEMENTS_READY and ELEMENT_READY. For efficiency it is advisiable that an elment signals NO_MORE_ELEMENTS
 * if it can. This allows an element collector to remove the source from its list of potential element sources.
 **/
public abstract class ElementSource {
    ...
    /**
     * Append an element source to this one. If this source is blocked or empty the appended source is checked
     */
    ...
    /**
     * This will step through all the elements in the ElementSource. Use this if the side effects matter, but you don't care about the actual values in the ElementSource.
     */
    ...
    /**
     * Get a Source of only the first element of the ElementSource without respecting its lazyness.
     */
    ...
    /**
     * Be aware that, as is usual with elementSources, getting elements which is implied by counting them, consumes the elements, so there is no way to get the elements after you
     * have counted them
     */
    ...
    /**
     * Users may call thi method to signify that they are done with this elementsource, possibly allowing the system to release resources needed to produce pending elements which
     * will never be consumed.
     * 
     * Default implementation does nothing, can be overridden by specific subclasses.
     */
    public void release() {
    }
