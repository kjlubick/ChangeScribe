...
/**
 * An ElementCollector collects and remembers elements from an element source. Theelement source is called upon when the collector needs to produce a new element.
 * 
 * ElementCollectors work in a lazy wya and will not request element sources to produce an element until the elements in the ElementCollector are being accessed.
 * 
 * Circular dependencies between ElementCollectors and ElementSources are handled by temporary locking ElementCollectors (see hurting flag) while they are trying to produce a new
 * element.
 */
...
public abstract class ElementCollector {
    ...
    /**
     * Create an ElementSource which produces the elements in this collector one by one. WARNING! Don't remove elements from a circularly dependent collector! Strange things happen
     * in combination with lazy evaluation.
     */
    ...
    /**
     * Collectors are lazy so you have to kick them to get them working on the next element. returns an ElementSource status flag.
     */
    ...
    /**
     * If I am kicked it hurts. If kicked again while hurting, this means I must be kicking myself indirectly. So I should not try to produce an element since I am already working
     * on one rigth now!
     */
    private boolean hurting = false;

    /**
     * Try to get an element from the source until you find a new one or it blocks. If an element is found it should be added using the "addElement" method. Returns if status flag
     * indicating succes or mode of failure for finding el
     */
...
class ElementCollectorSource extends RemovableElementSource {
    ...
    /**
     * Try to make an element ready if necessary and return the status after
     */
    @Override
    public int status() {
        ...
        if (pos.hasMoreElements()) {
            ...
            // myCollector==null: is forever depleted and already gone
