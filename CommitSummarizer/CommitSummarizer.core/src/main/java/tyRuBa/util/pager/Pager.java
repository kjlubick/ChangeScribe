...
/**
 * This is an object that allows operations to be performed on files. The responsibilty of the pager is to manage a queue of tasks and make sure that tasks are only executed when
 * the respective files are in memory. <br>
 * The pager is allowed to change the order in which tasks get executed, so it can postpone the execution of tasks for which the files have not been paged yet and give priority to
 * tasks that can be executed immediately.
 * 
 * @category FactBase
 * @author riecken
 */
public class Pager {
    ...
    public static abstract class ResourceId {
        ...
        /**
         * Check if the resource exists in its storage location. Note that this is not supposed to take into account potentially pending resource creation tasks in the DiskMan task
         * queue.
         * 
         * Therefore the only real safe way to check if a resource exists is to use the method on DiskMan (which calls this).
         */
    ...
    public static abstract class Task {
        ...
        /**
         * Performs a task on a resource. Note that rsrc can be null if the resource did not exist. In this case, the task may create a new resource, and then use the
         * changedResource method to notify the pager that there is a new resource.
         */
        public abstract Object doIt(Resource rsrc);

        /**
         * Task.doIt may call changeResource to provide a replacement for the current resource.
         * 
         * Task.doIt may also perform side effect directly on the resource, but in this case they *must* call changedResource so the pager knows the resource is dirty.
         */
    ...
    /**
     * Information about a reference that the pager uses to determine what gets paged out.
     */
    ...
    /**
     * Register last task time so we can detect if the QueryEngine is idle for some time and take advantage to do some stuff, like cleaning pages.
     */
    ...
    /**
     * Page cleaner is a thread which runs in background periodically checking if there are dirty resources and cleaning them (writing to disk) if so.
     */
    ...
    /**
     * Make the pager perform a task synchronously. A synchronous task has to wait until all other asynchronous tasks have been finished before it can run.
     */
    ...
    /**
     * Do a task asynchronously (currently not implemented, the task will perform as if it is a synchronous task.
     */
    ...
    /**
     * Crash is used for testing (to simulate a system crash). it kills the diskMan without a proper backup.
     */
    public void crash() {
        diskMan.crash();
    }
