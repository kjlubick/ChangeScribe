...
public class ASTVisitorAtomicChange extends ASTVisitor {
    ...
    private static String getSimpleName(IMethodBinding imb) {
        try {
            ...
            String args = "";
            /*
             * for (ITypeBinding itb : imb.getParameterTypes()) { if (args.length()>0) args+=","; args += getQualifiedName(itb); }
             */
            args = "(" + args + ")";
            return name + args;
        } catch (NullPointerException e) {
    ...
    public boolean visit(TypeDeclaration node) {
        ...
        /*
         * //make abstract method facts (if this is an interface) if (node.isInterface()) { try { List abstractbodies = node.bodyDeclarations(); for (Object o : abstractbodies) {
         * BodyDeclaration bd = (BodyDeclaration)o; System.out.println("aaa: "+bd .getNodeType()+" bbb: "+ASTNode.METHOD_DECLARATION); }
         * 
         * } catch (Exception e) { System.err.println("Cannot resolve abstract methods for interface " + node.getName().toString()); } }
         */
        return true;
    }
    ...
    public boolean visit(SimpleName node) {
        if (mtbStack.isEmpty() && !itbStack.isEmpty()) { // not part of a method
            return false;
            /*
             * try { return visitName(node.resolveBinding(), anonClassName.equals("")?getQualifiedName(itb):anonClassName); } catch (Exception e) {
    ...
    private boolean visitName(IBinding ib, IMethodBinding iMethodBinding)
        ...
        /*
         * IJavaElement ije = null; ije = ib.getJavaElement(); if (ije==null) return false; try { //if reference to a field if (ije instanceof ResolvedBinaryField || ije instanceof
         * ResolvedSourceField) { NamedMember nm = (NamedMember) ije; String fieldname = nm.getDeclaringType().getPackageFragment().getElementName()+
         * "%."+nm.getDeclaringType().getTypeQualifiedName('#')+ "#"+nm.getElementName(); facts.add(Fact.makeAccessesFact(fieldname, ownerFullName)); } else if (ije instanceof
         * ResolvedBinaryMethod || ije instanceof ResolvedSourceMethod) { //if reference is to a method //do nothing for now } } catch (Exception e) {
         * System.out.println("No good 2!"+ib.getName()); return false; } return false;
         */
    }
    ...
    public boolean visit(MethodInvocation node) {
        ...
        try {
            ...
            /*
             * && (mmtb.getName().equals("getPathIterator") || mmtb.getName().equals("lineTo") || mmtb.getName().equals("moveTo") || mmtb.getName().equals("closePath"))
             */) {
