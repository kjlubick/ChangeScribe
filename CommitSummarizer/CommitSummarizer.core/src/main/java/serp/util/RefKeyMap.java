...
/**
 * <p>
 * Abstract base class for maps whose keys are stored as weak or soft references. This class is more flexible than the standard {@link WeakHashMap} because it also allows soft
 * references and because it can inherit the functionality of any other map type. For example, it can use JVM identity-based hashing if constructed with an {@link IdentityMap} or
 * maintain its entries in insertion order if constructed with a {@link LinkedHashMap}.
 * </p>
 * 
 * <p>
 * Subclasses must define the {@link #createRefMapKey} method only. Expired values are removed from the map before any mutator methods; removing before accessor methods can lead to
 * {@link ConcurrentModificationException}s. Thus, the following methods may produce results which include key/value pairs that have expired:
 * <ul>
 ...
 *
 * <p>
 * By default, all methods are delegated to the internal map provided at construction. Thus, the ordering, etc of the given map will be preserved; however, the hashing algorithm
 * cannot be duplicated. A special case is made for the {@link IdentityMap}'s hashing, which is supported. Performance is similar to that of the internal map instance.
 * </p>
 *
 * @author Abe White
 */
        ...
        implements RefMap
{
    ...
    /**
     * Construct a RefKeyMap with the given interal map. The internal map will be cleared. It should not be accessed in any way after being given to this constructor; this map will
     * 'inherit' its behavior, however. For example, if the given map is a {@link LinkedHashMap}, the <code>values</code> method of this map will return values in insertion order.
     */
    ...
    /**
     * Create a weak or soft reference to hold the given key. In general, the returned reference should guarantee that its {@link Object#equals}, {@link Object#hashCode} and
     * {@link Comparable#compareTo} methods will be delegated to the given key value.
     *
     * @param key
     *            the key value to hold; will not be null
     * @param queue
     *            the reference queue to place the reference in, or null if the reference should not be placed in a queue
     * @param identity
     *            if true, the {@link Object#equals} and {@link Object#hashCode} methods of the returned reference should work by the JVM identity of the key, not its corresponding
     *            methods
     */
    ...
    /**
     * <p>
     * Represents a key held by weak or soft reference.
     * </p>
     *
     * @author Abe White
     */
