 ...
 * 
 * <p>
 * Subclasses must define the {@link #createRefValue} method only. Expired values are removed from the collection before any mutator methods; removing before accessor methods can
 * lead to {@link ConcurrentModificationException}s. Thus, the following methods may produce results which include values that have expired:
 * <ul>
 ...
 *
 * <p>
 * By default, all methods are delegated to the internal collection provided at construction. Thus, the ordering, etc of the given collection will be preserved. A special case is
 * also made for {@link MapSet}s using identity hashing, which is supported. Performance is similar to that of the internal collection instance.
 * </p>
 *
 * @author Abe White
 */
        ...
        implements RefCollection
{
    ...
    /**
     * Construct a RefCollection with the given interal collection. The internal collection will be cleared. It should not be accessed in any way after being given to this
     * constructor; this collection will 'inherit' its behavior, however. For example, if the given collection is a {@link TreeSet}, the elements will be maintained in natural
    ...
    /**
     * Tests if a new object and a stored value are equal according to the current comparison critieria (dependent on the identity setting).
     */
    ...
    /**
     * Create a weak or soft reference to hold the given value. In general, the returned reference should guarantee that its {@link Object#equals}, {@link Object#hashCode}, and
     * {#link Comparable#compareTo} methods will be delegated to the given value.
     *
     * @param value
     *            the value to hold; will not be null
     * @param queue
     *            the reference queue to place the reference in, or null if the reference should not be placed in a queue
     * @param identity
     *            if true, the {@link Object#equals} and {@link Object#hashCode} methods of the returned reference should work by the JVM identity of the value, not its
    ...
    /**
     * <p>
     * Represents a value held by weak or soft reference.
     * </p>
     *
     * @author Abe White
     */
