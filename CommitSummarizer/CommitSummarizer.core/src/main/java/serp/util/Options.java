...
/**
 * <p>
 * A specialization of the {@link Properties} map type with the added abilities to read application options from the command line and to use bean patterns to set an object's
 * properties via command-line the stored mappings.
 * </p>
 *
 * <p>
 * A typical use pattern for this class is to construct a new instance in the <code>main</code> method, then call {@link #setFromCmdLine} with the given args. Next, an instanceof
 * the class being invoked is created, and {@link #setInto} is called with that instance as a parameter. With this pattern, the user can configure any bean properties of the class,
 * or even properties of classes reachable from the class, through the command line.
 * </p>
 *
 * @author Abe White
 */
        ...
        extends TypedProperties
{
    ...
    /**
     * Parses the given argument list into flag/value pairs, which are stored as properties. Flags that are present without values are given the value "true". If any flag is found
     * for which there is already a mapping present, the existing mapping will be overwritten. Flags should be of the form:<br />
     * <code>java Foo -flag1 value1 -flag2 value2 ... arg1 arg2 ...</code>
     *
     * @param args
     *            the command-line arguments
     * @return all arguments in the original array beyond the flag/value pair list
    ...
    /**
     * This method uses reflection to set all the properties in the given object that are named by the keys in this map. For a given key 'foo', the algorithm will look for a
     * 'setFoo' method in the given instance. For a given key 'foo.bar', the algorithm will first look for a 'getFoo' method in the given instance, then will recurse on the return
     * value of that method, now looking for the 'bar' property. This allows the setting of nested object properties. If in the above example the 'getFoo' method is not present or
     * returns null, the algorithm will look for a 'setFoo' method; if found it will constrct a new instance of the correct type, set it using the 'setFoo' method, then recurse on
     * it as above. Property names can be nested in this way to an arbitrary depth. For setter methods that take multiple parameters, the value mapped to the key can use the ',' as
     * an argument separator character. If not enough values are present for a given method after splitting the string on ',', the remaining arguments will receive default values.
     * All arguments are converted from string form to the correct type if possible (i.e. if the type is primitive, java.lang.Clas, or has a constructor that takes a single string
     * argument). Examples:
     * <ul>
     ...
     * 		<code></li>
     * </ul>
     * Any keys present in the map for which there is no corresponding property in the given object will be ignored.
     *
     * @throws IllegalArgumentException
     *             on parse error
     */
    ...
    /**
     * Sets the property named by the key of the given entry in the given object.
     */
     ...
     *
     * @param key
     *            the key given at the command line; may be of the form 'foo.bar' to signify the 'bar' property of the 'foo' owned object
     * @param match
     *            an array of length 2, where the first index is set to the object to retrieve the setter for
     * @return true if a match was made, false otherwise; additionally, the first index of the match array will be set to the matching object and the second index will be set to
     *         the setter method for the property named by the key
     */
    ...
    /**
     * Converts the given string into an object of the given type, or its wrapper type if it is primitive.
     */
