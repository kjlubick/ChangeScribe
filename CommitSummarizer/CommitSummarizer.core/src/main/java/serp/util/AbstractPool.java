...
/**
 * <p>
 * Abstract generic pool implementation. Subclasses must implement the {@link #freeSet} and {@link #takenMap} methods to return mutable views of the free and taken pool instances,
 * respectively.
 * </p>
 *
 * <p>
        ...
        implements Pool
{
    ...
    /**
     * Construct an empty pool with <code>min, max, wait, autoReturn</code> properties of 0.
     */
    public AbstractPool()
    {
    }
     ...
     *
     * @param min
     *            the minimum pool size, including taken instances
     * @param max
     *            the maximum pool size, including taken instances
     * @param wait
     *            the maximum number of milliseconds to wait for a free instance
     * @param autoReturn
     *            the number of milliseconds after which taken instances can be automatically reclaimed
     */
    ...
    /**
     * Return a free object from the pool that compares true using {@link Object#equals} to the given instance.
     *
     * @see #get(Object,Comparator)
     */
    @Override
    ...
    /**
     * Return a free object matching from the pool. The object must match the given instance according to the given {@link Comparator}. All other <code>get</code> methods are
     * implemented in terms of this version.
     *
     * @param match
     *            the object to compare to; may be null
     * @param comp
     *            the comparator to use; if null and <code>match</code> is null, any object will match; if null and <code>match</code> is not null, matching will be based on the
     *            {@link Object#equals} method
     * @throws NoSuchElementException
     *             if no matching object can be obtained in the set wait period
     */
    @Override
    ...
    /**
     * Locates a pooled object that matches the given instance according to the given comparator. The comparator will always be valid. This method is used by {@link #get} to find
     * matching instances. The pool is first cleaned via the {@link #clean} method, then if the free set has any elements in it, it is iterated to find a matching instance. Beore
     * returning any matches the taken object is placed in the taken set along with the current time.
     *
    ...
    protected void clean()
    {
        ...
        if (_autoReturn > 0)
        {
            ...
            long now = System.currentTimeMillis();
            Map.Entry entry;
            for (Iterator itr = takenMap().entrySet().iterator(); itr.hasNext();)
            {
    ...
    /**
     * Provide a modifiable map view of the taken pool instances. Each entry will be used to map a taken instance to a {@link Long} value representing the millisecond time at which
     * the key was taken. Implementations are free to use weak mappings.
     */
