...
public class LSdiffHierarchialDeltaKB {
    ...
    private void printPakcageLevelFactStat(PrintStream p) {
        if (p != null)
            p.println("# added_package:\t" + packageLevel.get(ADDED).size());
        if (p != null)
            p.println("# deleted_package:\t" + packageLevel.get(DELETED).size());
        if (p != null)
            p.println("# modified_package:\t" + packageLevel.get(MODIFIED).size());
    ...
    private void printTypeLevelFactStat(PrintStream p) {
        if (p != null)
            p.println("# added_type:\t" + typeLevel.get(ADDED).size());
        if (p != null)
            p.println("# deleted_type:\t" + typeLevel.get(DELETED).size());
        if (p != null)
            p.println("# modified_type:\t" + typeLevel.get(MODIFIED).size());
    }

    @SuppressWarnings("unused")
    private void printMethodLevelFactStat(PrintStream p) {
        if (p != null)
            p.println("# added_method:\t" + methodLevel.get(ADDED).size());
        if (p != null)
            p.println("# deleted_method:\t" + methodLevel.get(DELETED).size());
        if (p != null)
            p.println("# modified_method:\t" + methodLevel.get(MODIFIED).size());
    }

    @SuppressWarnings("unused")
    private void printFieldLevelFactStat(PrintStream p) {
        if (p != null)
            p.println("# added_field:\t" + fieldLevel.get(ADDED).size());
        if (p != null)
            p.println("# deleted_field:\t" + fieldLevel.get(DELETED).size());
        if (p != null)
            p.println("# modified_field:\t" + fieldLevel.get(MODIFIED).size());
