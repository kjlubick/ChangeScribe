...
public class StereotypedType extends TypeStereotypeRules implements StereotypedElement {
    ...
    static {
        StereotypedType.TYPE_KEY_PATTERN = Pattern
                .compile("([A-Z])(([a-zA-Z_][a-zA-Z0-9_]+/)*)([a-zA-Z_][a-zA-Z0-9_]+)(~([a-zA-Z0-9_]+))?((\\$[a-zA-Z_][a-zA-Z0-9_]+)*)(\\$[0-9]+)?;");
    }
    ...
    private void completeReport() {
        /*
         * this.report.append("\n\nMethod counters (without considering constructors and methods overriding Object's methods)"); this.report.append("\nTOTAL" + Reporter.SEPARATOR +
         * this.totalMethods.size()); this.report.append("\n" + MethodStereotype.Category.ACCESSOR + Reporter.SEPARATOR + this.accessorMethods.size()); this.report.append("\n" +
         * MethodStereotype.Category.MUTATOR + Reporter.SEPARATOR + this.mutatorMethods.size()); this.report.append("\n" + MethodStereotype.Category.COLLABORATIONAL +
         * Reporter.SEPARATOR + this.collaborationalMethods.size()); this.report.append("\n" + MethodStereotype.Category.DEGENERATE + Reporter.SEPARATOR +
         * this.degenerateMethods.size()); this.report.append("\n" + MethodStereotype.GET + Reporter.SEPARATOR + this.getMethods.size()); this.report.append("\n" +
         * MethodStereotype.PREDICATE + Reporter.SEPARATOR + this.predicateMethods.size()); this.report.append("\n" + MethodStereotype.PROPERTY + Reporter.SEPARATOR +
         * this.propertyMethods.size()); this.report.append("\n" + MethodStereotype.VOID_ACCESSOR + Reporter.SEPARATOR + this.voidAccessorMethods.size()); this.report.append("\n" +
         * MethodStereotype.SET + Reporter.SEPARATOR + this.setMethods.size()); this.report.append("\n" + MethodStereotype.COMMAND + Reporter.SEPARATOR +
         * this.commandMethods.size()); this.report.append("\n" + MethodStereotype.NON_VOID_COMMAND + Reporter.SEPARATOR + this.nonVoidCommandMethods.size());
         * this.report.append("\n" + MethodStereotype.FACTORY + Reporter.SEPARATOR + this.factoryMethods.size()); this.report.append("\n" + MethodStereotype.LOCAL_CONTROLLER +
         * Reporter.SEPARATOR + this.localControllerMethods.size()); this.report.append("\n" + MethodStereotype.COLLABORATOR + Reporter.SEPARATOR +
         * this.collaboratorMethods.size()); this.report.append("\n" + MethodStereotype.CONTROLLER + Reporter.SEPARATOR + this.controllerMethods.size()); this.report.append("\n" +
         * MethodStereotype.INCIDENTAL + Reporter.SEPARATOR + this.incidentalMethods.size()); this.report.append("\n" + MethodStereotype.EMPTY + Reporter.SEPARATOR +
         * this.emptyMethods.size()); this.report.append("\n" + MethodStereotype.ABSTRACT + Reporter.SEPARATOR + this.abstractMethods.size()); final StringBuilder typeInfo = new
         * StringBuilder(String.valueOf(this.getKey()) + Reporter.SEPARATOR + Reporter.SEPARATOR); if (this.primaryStereotype != null) {
         * typeInfo.append(String.valueOf(Reporter.SEPARATOR) + this.primaryStereotype); } if (this.secondaryStereotype != null) {
         * typeInfo.append(String.valueOf(Reporter.SEPARATOR) + this.secondaryStereotype); } typeInfo.append("\n"); this.report.insert(0, typeInfo.toString());
         */
    }
