...
public class SummarizeChangesTMP {
        ...
        Job job = new Job("Calculating method and types stereotypes") {
            ...
            protected IStatus run(IProgressMonitor externalMonitor) {
                for (final ChangedFile file : differences) {
                    ...
                    /*
                     * internalJob.addJobChangeListener(new JobChangeAdapter() { public void done(IJobChangeEvent event) { updateTextInputDescription(); } });
                     */
    ...
    public void updateTextInputDescription() {
        ...
        Display.getDefault().asyncExec(new Runnable() {
            ...
            public void run() {
                /*
                 * if(summarized.size() + modifiedFiles.size() + otherFiles.size() + typesProblem.size() == differences.length || summarized.size() + modifiedFiles.size() +
                 * otherFiles.size() + typesProblem.size() == differences.length) {
                 */
                ...
                for (Entry<String, StereotypeIdentifier> identifier : summarized.entrySet()) {
                    ...
                    if (identifier.getValue().getScmOperation().equals(TypeChange.MODIFIED.toString())) {
                        ...
                        modificationDescriptor.extractDifferencesBetweenVersions(identifier.getValue().getChangedFile(), git, changedListDialog.getAuthorText().getText(),
                                changedListDialog.getCommitterText().getText());
                        ...
                        if (!identifier.getValue().getChangedFile().isRenamed()) {
                            ...
                            descTmp.append((i - 1)
                                    ...
                                    + ". "
                                    + "Rename type "
                                    + identifier.getValue().getChangedFile().getRenamedPath()
                                            .substring(identifier.getValue().getChangedFile().getRenamedPath().lastIndexOf("/") + 1).replace(".java", "") + " with "
    ...
    public String summarizeCommitStereotype() {
        ...
        if (methods.size() > 0) {
            ...
            if (stereotype != null) {
               ...
               * else { result = "Not found commit stereotype. "; }
               */
     ...
     * 
     * Set<ICompilationUnit> previousCU = new HashSet<>(); Set<ICompilationUnit> currentCU = new HashSet<>(); IPackageFragment[] packages; try { packages =
     * changedListDialog.getSelection().getPackageFragments(); for (IPackageFragment mypackage : packages) { if (mypackage.getKind() == IPackageFragmentRoot.K_SOURCE) { for
     * (ICompilationUnit unit : mypackage.getCompilationUnits()) { ChangedFile file = new ChangedFile(unit.getPath().toString().replaceFirst("/", ""), TypeChange.ADDED.name(),
     * getGit().getRepository().getWorkTree().getAbsolutePath()); IndexDiff diff = null; try { diff = new IndexDiff(git.getRepository(), "HEAD", new
     * FileTreeIterator(git.getRepository())); diff.setFilter(new PathFilterGroup().createFromStrings(file.getPath())); diff.diff(); } catch (IOException e) { // TODO
     * Auto-generated catch block e.printStackTrace(); }
     * 
     * if (!diff.getModified().isEmpty() || !diff.getRemoved().isEmpty()) { previousCU.add(findRefactorings(file)); }
     * 
     * } Set<ChangedFile> differences = SCMRepository.getRemovedFiles(git.status().call(), getGit().getRepository().getWorkTree().getAbsolutePath()); for (ChangedFile file :
     * differences) { previousCU.add(findRefactorings(file)); } System.out.println("Source Name " + mypackage.getElementName()); System.out.println("Number of Classes: " +
     * mypackage.getClassFiles().length);
     * 
     * //findRefactorings(file); }
     * 
     * } } catch (JavaModelException | NoWorkTreeException | GitAPIException e) { e.printStackTrace(); }
     * 
     * try { currentCU = getFiles("ChangeDiff"); } catch (CoreException e) { // TODO Auto-generated catch block e.printStackTrace(); }
     * 
     * List<LSDResult> result = (new LSDiffExecutor()).doLSDiff(currentCU, previousCU); if(result != null && !result.isEmpty()) { //rules.addAll(result); } }
     * 
     * private static Set<ICompilationUnit> getFiles(String projname) throws CoreException { IWorkspaceRoot ws = ResourcesPlugin.getWorkspace().getRoot(); IProject proj =
     * ws.getProject(projname); IJavaProject javaProject = JavaCore.create(proj); Set<ICompilationUnit> files = new HashSet<ICompilationUnit>(); javaProject.open(new
     * NullProgressMonitor()); for( IPackageFragment packFrag : javaProject.getPackageFragments()) { for (ICompilationUnit icu : packFrag.getCompilationUnits()) { files.add(icu); }
     * } javaProject.close(); return files; }
     * 
     * protected ICompilationUnit findRefactorings(ChangedFile file) { IProject project = createProject(); IFolder src = null; IJavaProject javaProject = null; ICompilationUnit cu
     * = null; try { javaProject = JavaCore.create(project); javaProject.open(null); src = project.getFolder("src"); if(!src.exists()) { src.create(true , true , null); }
     * 
     * String removedFile;
     * 
     * removedFile = Utils.getStringContentOfLastCommit(file.getPath(), getGit().getRepository()); IPackageFragment pack = null; String packageName = ""; packageName =
     * CompilationUtils.getPackageNameFromStringClass(removedFile); IPackageFragmentRoot rootPack = javaProject.getPackageFragmentRoot(src);
     * 
     * pack = rootPack.createPackageFragment(packageName, true, null); cu = pack.createCompilationUnit(file.getName(), removedFile,true, null); stereotypeIdentifier = new
     * StereotypeIdentifier(cu, 0, 0);
     * 
     * } catch (RevisionSyntaxException | IOException | IllegalStateException e) { e.printStackTrace(); } catch (CoreException | NullPointerException e) { //deleteTmpProject();
     * e.printStackTrace(); } return cu; }
     * 
     * rotected void deleteTmpProject() { IProject project = createProject(); try { project.delete(true, null); } catch (CoreException e) { // TODO Auto-generated catch block
     * e.printStackTrace(); } } public IProject createProject() { IProject project = null; IFolder src = null; IJavaProject javaProject = null; try { IWorkspaceRoot root =
     * ResourcesPlugin.getWorkspace().getRoot(); project = root.getProject("MyProject");
     * 
     * if(!project.exists()) { project.create(null); } project.open(null);
     * 
     * src = project.getFolder("src"); if(!src.exists()) { src.create(false , true , null); }
     * 
     * IProjectDescription description = project.getDescription(); String[] natures = description.getNatureIds(); String[] newNatures = new String[natures.length + 1];
     * System.arraycopy(natures, 0, newNatures, 0, natures.length); newNatures[natures.length] = JavaCore.NATURE_ID; description.setNatureIds(newNatures);
     * project.setDescription(description, null);
     * 
     * javaProject = JavaCore.create(project); javaProject.open(null);
     * 
     * IPackageFragmentRoot rootPack = javaProject.getPackageFragmentRoot(src);
     * 
     * IClasspathEntry[] oldEntries = javaProject.getRawClasspath(); IClasspathEntry[] newEntries = new IClasspathEntry[1]; //System.arraycopy(oldEntries, 0, newEntries, 0,
     * oldEntries.length); newEntries[0] = JavaCore.newSourceEntry(rootPack.getPath()); javaProject.setRawClasspath(newEntries, null);
     * 
     * } catch (CoreException | RevisionSyntaxException e) { e.printStackTrace(); } return project; }
     */
